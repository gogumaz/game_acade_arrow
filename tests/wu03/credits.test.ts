// 크레딧 스텁 (§10.1~§10.4) — 상한 · 혼합 결제 금지 · 차감/원복 (작업 계획 §8.1)
//
// 통계·`credit_log.csv`·날짜 롤오버·크래시 복구(CRD-604~607)는 **WU-04** 범위다.
...
import {
  ...
  type CreditsStub,
  type RefundRecord,
} from '../../src/game/creditsStub';
import { creditsSpy, insertCoins } from './harness';
...
describe('코인 적립과 상한 (§10.3)', () => {
  ...
  it('이벤트 크레딧 상한은 5다', () => {
    const credits: CreditsStub = createCreditsStub();
  ...
  it('이벤트 시작 잔액은 0이다', () => {
    expect(createCreditsStub().balance().event).toBe(0);
  });
...
describe('시작·컨티뉴 판정 (§2.7 · §4.5)', () => {
  it('크레딧이 C 미만이면 시작할 수 없다', () => {
    ...
    const credits = createCreditsStub(cfg);
    credits.insertCoin();
  ...
  it('컨티뉴 비용은 시작 비용과 따로 설정된다', () => {
    const credits = createCreditsStub({ coinsPerPlay: 3, continueCoins: 1 });
    credits.insertCoin();
    expect(credits.canStart()).toBe(false);
  ...
  it('차감하면 잔액이 정확히 줄어든다', () => {
    const credits = createCreditsStub({ coinsPerPlay: 2 });
  ...
  it('컨티뉴 차감도 같은 규칙이다', () => {
    const credits = createCreditsStub({ continueCoins: 2 });
...
describe('§10.4 혼합 결제 금지 (CRD-603의 스텁분)', () => {
  ...
  it('이벤트가 부족하면 섞지 않고 PAID 단독으로 낸다', () => {
    const credits = createCreditsStub({ coinsPerPlay: 3 });
  ...
  it('둘 다 부족하면 결제하지 않는다', () => {
    const credits = createCreditsStub({ coinsPerPlay: 4 });
    credits.grantEvent(2);
    insertCoins(credits, 3);
    expect(credits.chargeStart()).toBe('none');
...
describe('§10.2 원복', () => {
  ...
  it('원복 사유가 기록된다 (WU-04가 credit_log.csv로 옮긴다)', () => {
    const credits = createCreditsStub();
    insertCoins(credits, 1);
    credits.chargeStart();
    credits.refund(1, 'paid', '보드 생성 실패');
    const expected: readonly RefundRecord[] = [
      { amount: 1, source: 'paid', reason: '보드 생성 실패' },
    ];
    expect(credits.refunds).toEqual(expected);
  });
});
